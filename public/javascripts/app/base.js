...
KISSY.app('H', function() {
    ...
    return {
		...
		util: {
			...
			/*
			 * ����data.responseText, s === data.responseText
			 * @return {Object}
			 * */
...
<div class="ui-dialog ui-dialog-dd">
	...
	<div class="ui-dialog-ft"><a class="close" href="#close" title="�ر�"></a></div>
</div>
...
*/	
		defConfig = {
			ID: null,
			head: 'Title',
			body: '<div class="ui-dialog-loading">���ڼ��أ����Ժ�...</div>',
 ...
 */
 H.add('widget~msg', function( HLG ) {
	...
	S.mix(Msg.prototype, {
		...
		/*һ��������Ϣ��ʾ body ��ʽ
		 * <div class="point relative"><div class="point-w-1">�Ź���ʼ���������������ڵ���0</div></div>
		 * */
		showDialog: function(){
			var self = this;
			if(!this.panel){
        ...
        dispatchResponse: function(o,b) {
            ...
            if (onload) {
                try { (new Function(onload))();
                   ...
                   // HLG.widget.msgBox.setMsg('ִ�з��������еĽű�����').setAutohide().show()
                }
            }
        },
            ...
            switch (mode) {
				...
				case 2: 
					...
					if(this.pageCount > 1){
	                    ...
	                    if (prevPage < 1) {
	                        ...
	                        strHtml += '<a class="" href="javascript:' + self.name + '.toPage(' + prevPage + ');" title="��һҳ"><span class="pre page-pic-no"></span></a>';
	                      }
	                    if (this.page != 1) {
						...
						if (nextPage > this.pageCount) {
	                        ...
	                        strHtml += '<a class="" href="javascript:' + self.name + '.toPage(' + nextPage + ');" title="��һҳ"><span class="next page-pic-no"></span></a>';
	                      }
						 if (this.pageCount > 5) {
			   					strHtml += '<font class="number">';
			   					strHtml += '��'+this.pageCount+'ҳ&nbsp;����&nbsp;';
			   					if(this.page>=this.pageCount){
			   						strHtml += '<input style="" type="text" class="page-pic-no w-30 bg-img" id="pageInput' + self.name + '"  value="' + this.pageCount + '" onkeypress="return ' + self.name + '.formatInputPage(event);" onfocus="this.select()">&nbsp;ҳ';
			   					}else{
			   						strHtml += '<input style="" type="text" class="page-pic-no w-30 bg-img" id="pageInput' + self.name + '"  value="' + (this.page+1) + '" onkeypress="return ' + self.name + '.formatInputPage(event);" onfocus="this.select()">&nbsp;ҳ';
			   					}
			   ...
			   case 3 :
                   ...
                   if (prevPage < 1) {
                       ...
                       strHtml += '<a class="border-left-dedede" href="javascript:' + self.name + '.toPage(' + prevPage + ');" title="��һҳ"><span class="pre page-pic-no"></span></a>';
                     }
                   if (nextPage > this.pageCount) {
                   	strHtml += '<span class="next-none page-pic-no"></span>';
                   } else {
                       strHtml += '<a href="javascript:' + self.name + '.toPage(' + nextPage + ');" title="��һҳ"><span class="next page-pic-no"></span></a>';
                     }
                  strHtml += '<div style="clear:both"></div></div></div>';
                  break;
...
H.add('widget~countdown', function( HLG ) { 
	...
	var S = KISSY, DOM = S.DOM, Event = S.Event, doc = document;
	function countdown(contain, endTime, mode) { //��ʼ������ 
		var self = this; 
	...
	S.mix(countdown.prototype,{
		init: function(contain, endTime, mode) {
 	    	//var n = endTime || 1440; //ʣ��������
 	    		var self = this;
			if (mode == '' || typeof(mode) == 'undefined') mode = 1;
			if(mode == 1){
				// �� ʱ �� ���� 
				DOM.html(DOM.get(contain),' <span class="day"></span>��<span class="hour"></span>ʱ<span class="min"></span>��<span class="sec"></span>�� ');
			} 
			if(mode == 2){
				//  ʱ �� ���� 
				DOM.html(DOM.get(contain),' <span class="hour">19</span>ʱ<span class="min">19</span>��<span class="sec">26</span>��');
			} 
			if(mode == 3){
				//  ʱ �� ���� 
...
H.add('widget~roll', function( HLG ) { 
	...
	*/	
	...
	function roll(contain, mode,speed) { //��ʼ������ 
		var self = this;
			...
			switch(mode){
				...
				case DOWN: 
					...
					function MarqueeDown(){
						if(self.tab1.offsetTop-self.tab.scrollTop>=0)//��������demo1��demo2����ʱ
							self.tab.scrollTop+=self.tab2.offsetHeight;//demo�������
						else{
							self.tab.scrollTop--;
						}
					}
